...
diurnals_2019_A_vs_leafT_licor_first_HW_r_3_4_trial<-diurnals_2019_A_vs_leafT_licor_first_HW_r_3_4%>%
  filter(treatment == 2)

diurnals_2019_A_vs_leafT_licor_first_HW_r_3_4_trial%>%
  tally()

lm.trial <- lm(diurnals_2019_A_vs_leafT_licor_first_HW_r_3_4_trial$A ~poly( diurnals_2019_A_vs_leafT_licor_first_HW_r_3_4_trial$Tleaf, 2))

summary(lm.trial)
summary(lm.trial$model)
confint(lm.trial)
summary.lm(lm.trial)
predict.lm(lm.trial)
...
A_vs_leafT_licor_first_HW_r_3_4_trial <-ggplot(diurnals_2019_A_vs_leafT_licor_first_HW_r_3_4_trial, aes(Tleaf, A))+
  geom_point()

y <- diurnals_2019_A_vs_leafT_licor_first_HW_r_3_4_trial$A
x <- diurnals_2019_A_vs_leafT_licor_first_HW_r_3_4_trial$Tleaf
poly2trail <- lm(y ~poly(x, 2))
xc <- seq(30, 47, by=1 )
dfc <- data.frame(x=xc)
dfc$poly2trail <- predict(poly2trail,dfc)
print(A_vs_leafT_licor_first_HW_r_3_4_trial +   geom_line(data=dfc,aes(x,poly2trail), size=1, colour="#339900"))+
  stat_regline_equation(
  aes(label =  paste(..eq.label.., ..rr.label.., sep = "~")),
  formula = y ~ poly(x,2), size = 3.5
)
...
physio_respone_fit <- lm(A ~ poly(Tleaf, 2), data = diurnals_2019_A_vs_leafT_licor_first_HW_r_3_4_trial)
...
physio_respone_fit <- predict(physio_respone_fit,diurnals_2019_A_vs_leafT_licor_first_HW_r_3_4_trial, interval = "confidence")

summary(physio_respone_fit)

diurnals_2019_A_vs_leafT_licor_first_HW_r_3_4_trial <- diurnals_2019_A_vs_leafT_licor_first_HW_r_3_4_trial %>% 
  mutate(fit = physio_respone_fit[,"fit"],
         lwr = physio_respone_fit[,"lwr"],
         upr = physio_respone_fit[,"upr"])

A_vs_leafT_licor_first_HW_r_3_4_trial_2 <-ggplot(diurnals_2019_A_vs_leafT_licor_first_HW_r_3_4_trial, aes(Tleaf, A)) +
  geom_point(alpha =0.6, size = 1.7 ) + 
...
summary(physio_respone_fit)
physio_respone_fit_pred <- predict(physio_respone_fit,diurnals_2019_A_vs_leafT_licor_first_HW_r_3_4_trial, interval = "prediction")
